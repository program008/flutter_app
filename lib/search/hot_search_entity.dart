class HotSearchEntity {
	...
	HotSearchEntity.fromJson(Map<String, dynamic> json) {
		if (json['data'] != null) {
			data = new List<HotSearchData>();(json['data'] as List).forEach((v) { data.add(new HotSearchData.fromJson(v)); });
		}
...
class HotSearchData {
	...
	HotSearchData({this.visible, this.link, this.name, this.id, this.order});
